    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 text-white flex flex-col">
      <div className="flex-grow flex flex-col">
        ...
        <header className="p-4 md:p-8 bg-gray-900">
          ...
          {/* Navigation Tabs */}
          <nav className={`md:flex md:justify-start border-b border-gray-600 ${isMobileMenuOpen ? 'block' : 'hidden md:flex'}`}>
            {navItems.map((tab) => (
              <button
                key={tab}
                onClick={() => {
                ...
                `}
              >
                {tab === 'addFriend' ? 'Add Friend' : tab.charAt(0).toUpperCase() + tab.slice(1)}
              </button>
            ))}
          </nav>
        </header>
